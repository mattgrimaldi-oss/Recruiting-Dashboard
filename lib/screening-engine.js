...
function buildCriteriaPrompt(roleConfig) {
  ...
  const informational = criteria.filter(c => c.status !== 'required');

  const sections = [`Today's date is ${TODAY}.`];

  if (required.length > 0) {
    const reqLines = required.map((c, i) => `${i + 1}. ${c.label.toUpperCase()} — ${c.detail}`);
    sections.push(`REQUIRED CRITERIA — candidate must meet ALL of these to pass:\n${reqLines.join('\n')}`);
  } else {
    sections.push('There are no required criteria. The candidate passes automatically.');
  }

  if (informational.length > 0) {
    const infoLines = informational.map((c, i) => `- ${c.label.toUpperCase()} — ${c.detail}`);
    sections.push(`INFORMATIONAL ONLY — note these but they do NOT affect pass/fail:\n${infoLines.join('\n')}`);
  }
...
function buildJsonShape(roleConfig) {
  ...
  const infoFields = informational.map(c =>
...
for (const [roleKey, roleConfig] of Object.entries(CRITERIA_FILE).filter(([k]) => k !== 'config')) {
  JOB_CONFIGS[roleKey] = {
    ...
    requiredCount: roleConfig.criteria.filter(c => c.status === 'required').length,
...
async function processJob(roleKey, timeWindowHoursOverride, { silent = false } = {}) {
  ...
  for (const app of eligible) {
    ...
    if (!score.passes) console.log(`   📋 ${candidateName} score:`, JSON.stringify(score, null, 2));
