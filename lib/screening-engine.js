...
async function processJob(roleKey, timeWindowHoursOverride) {
  ...
  for (const app of eligible) {
    ...
    if (score.passes) {
      const location = candidate.addresses?.[0]?.value
        || (typeof candidate.location === 'string' ? candidate.location : candidate.location?.name)
        || app.location?.name
        || null;
