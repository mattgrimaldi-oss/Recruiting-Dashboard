const pdfParse = require('pdf-parse');
const mammoth = require('mammoth');
const path = require('path');

const TODAY = new Date().toISOString().slice(0, 10);

// Load criteria from criteria.json (editable via dashboard UI)
const CRITERIA_PATH = path.join(__dirname, '..', 'criteria.json');
const CRITERIA_FILE = require(CRITERIA_PATH);

function buildCriteriaPrompt(roleConfig) {
  const { criteria } = roleConfig;
  const required    = criteria.filter(c => c.status === 'required');
  const bonus       = criteria.filter(c => c.status === 'bonus');
  const notRequired = criteria.filter(c => c.status === 'not_required');

  const sections = [`Today's date is ${TODAY}.`];

  if (required.length > 0) {
    const reqLines = required.map((c, i) => `${i + 1}. ${c.label.toUpperCase()} — ${c.detail}`);
    sections.push(`REQUIRED — candidate must meet ALL of these to pass:\n${reqLines.join('\n')}`);
  } else {
    sections.push('There are no required criteria. The candidate passes automatically.');
  }

  if (bonus.length > 0) {
    const bonusLines = bonus.map(c => `- ${c.label.toUpperCase()} — ${c.detail}`);
    sections.push(`BONUS — note these as positive signals but they do NOT affect pass/fail:\n${bonusLines.join('\n')}`);
  }

  if (notRequired.length > 0) {
    const nrLines = notRequired.map(c => `- ${c.label.toUpperCase()} — ${c.detail}`);
    sections.push(`NOT REQUIRED — note these for context but they do NOT affect pass/fail:\n${nrLines.join('\n')}`);
  }

  return sections.join('\n\n');
}

function buildJsonShape(roleConfig) {
  const { criteria } = roleConfig;
  const required = criteria.filter(c => c.status === 'required');
  const bonus = criteria.filter(c => c.status === 'bonus');
  const notRequired = criteria.filter(c => c.status === 'not_required');

  const reqFields = required.map(c =>
    `    "${c.key}": { "met": true or false, "detail": "one concise sentence" }`
  ).join(',\n');

  const bonusFields = bonus.map(c =>
    `    "${c.key}": "one concise sentence noting what you found"`
  ).join(',\n');

  const nrFields = notRequired.map(c =>
    `    "${c.key}": "one concise sentence noting what you found"`
  ).join(',\n');

  let shape = `{\n  "passes": true or false,\n  "location": "city, state from resume or null if not found"`;
  if (reqFields) shape += `,\n  "criteria": {\n${reqFields}\n  }`;
  if (bonusFields) shape += `,\n  "bonus": {\n${bonusFields}\n  }`;
  if (nrFields) shape += `,\n  "notes": {\n${nrFields}\n  }`;
  shape += `,\n  "summary": "1 sentence — why they pass or the main reason they fail",\n  "highlights": ["most impressive thing", "second most impressive thing"]\n}`;
  return shape;
}

// A resume cannot tell you whether someone was rejected in Greenhouse before, so
// asking the model to judge it just makes it guess (it was failing candidates with
// "no information available to confirm"). Detect that criterion and enforce it
// against the ATS instead, excluding it from the prompt entirely.
function isPriorRejectionCriterion(c) {
  const text = `${c.label || ''} ${c.detail || ''}`.toLowerCase();
  return /reject/.test(text) && /(greenhouse|previously|before|prior|at all)/.test(text);
}

// Build JOB_CONFIGS dynamically from criteria.json
const globalCriteria = CRITERIA_FILE._global?.criteria || [];
const JOB_CONFIGS = {};
for (const [roleKey, roleConfig] of Object.entries(CRITERIA_FILE).filter(([k]) => k !== 'config' && k !== '_global')) {
  const roleSpecificKeys = new Set(roleConfig.criteria.map(c => c.key));
  const filteredGlobal = globalCriteria.filter(c => !roleSpecificKeys.has(c.key));
  const allCriteria = [...filteredGlobal, ...roleConfig.criteria];

  const rejectionCriterion = allCriteria.find(isPriorRejectionCriterion) || null;
  // Everything the model still judges from the resume.
  const llmCriteria = allCriteria.filter(c => !isPriorRejectionCriterion(c));
  const mergedConfig = { ...roleConfig, criteria: llmCriteria };

  JOB_CONFIGS[roleKey] = {
    jobIds: Array.isArray(roleConfig.jobIds) ? roleConfig.jobIds : [roleConfig.jobId],
    jobName: roleConfig.jobName,
    // Only enforce in code when the recruiter marked it required.
    enforcePriorRejection: !!rejectionCriterion && rejectionCriterion.status === 'required',
    requiredCount: mergedConfig.criteria.filter(c => c.status === 'required').length,
    criteriaPrompt: buildCriteriaPrompt(mergedConfig),
    criteriaKeys: mergedConfig.criteria.map(c => ({
      key: c.key,
      label: c.label,
      ...(c.status !== 'required' ? { informational: true } : {}),
    })),
    jsonShape: buildJsonShape(mergedConfig),
  };
}

// ─── Greenhouse ───────────────────────────────────────────────────────────────

async function fetchGreenhouse(path) {
  const auth = Buffer.from(`${process.env.GREENHOUSE_API_KEY_V1}:`).toString('base64');
  const res = await fetch(`https://harvest.greenhouse.io/v1${path}`, {
    headers: { 'Authorization': `Basic ${auth}` },
  });
  if (!res.ok) throw new Error(`Greenhouse ${path}: ${res.status}`);
  return res.json();
}

async function fetchGreenhouseAll(path) {
  const auth = Buffer.from(`${process.env.GREENHOUSE_API_KEY_V1}:`).toString('base64');
  const all = [];
  let url = `https://harvest.greenhouse.io/v1${path}`;
  while (url) {
    const res = await fetch(url, { headers: { 'Authorization': `Basic ${auth}` } });
    if (!res.ok) throw new Error(`Greenhouse ${path}: ${res.status}`);
    const data = await res.json();
    if (Array.isArray(data)) all.push(...data);
    const link = res.headers.get('link') || '';
    const next = link.match(/<([^>]+)>;\s*rel="next"/);
    url = next ? next[1] : null;
  }
  return all;
}

// ─── Prior rejection (ATS-enforced) ───────────────────────────────────────────

// Greenhouse knows the candidate's full application history. GET /candidates/:id
// usually embeds it, so prefer that (no extra request) and only fall back to the
// dedicated endpoint if it's missing.
async function getPriorRejections(candidate, currentApplicationId) {
  let apps = Array.isArray(candidate.applications) ? candidate.applications : null;
  if (!apps) {
    try {
      const fetched = await fetchGreenhouse(`/candidates/${candidate.id}/applications`);
      apps = Array.isArray(fetched) ? fetched : [];
    } catch (e) {
      console.error(`   ⚠️  could not load application history for ${candidate.id}: ${e.message}`);
      return null; // unknown — don't reject on missing data
    }
  }
  return apps.filter(a => a.status === 'rejected' && a.id !== currentApplicationId);
}

// ─── Work authorization ───────────────────────────────────────────────────────

function isWorkAuthorized(application) {
  const answers = application.answers || [];
  const authAnswer = answers.find(a =>
    a.question?.toLowerCase().includes('authorized') ||
    a.question?.toLowerCase().includes('sponsorship') ||
    a.question?.toLowerCase().includes('work authorization')
  );
  if (!authAnswer) return { authorized: true, flagForReview: true };
  return { authorized: authAnswer.answer?.toLowerCase().includes('yes'), flagForReview: false };
}

// ─── Resume parsing ───────────────────────────────────────────────────────────

async function parseResume(candidate) {
  const attachment = candidate.attachments?.find(a => a.type === 'resume');
  if (!attachment) return null;
  const fileRes = await fetch(attachment.url);
  if (!fileRes.ok) return null;
  const contentType = fileRes.headers.get('content-type') || '';
  const buffer = Buffer.from(await fileRes.arrayBuffer());
  const name = (attachment.filename || '').toLowerCase();

  if (contentType.includes('pdf') || name.endsWith('.pdf')) {
    const parsed = await pdfParse(buffer);
    return parsed.text;
  }

  // .docx is a zip archive — reading it as utf-8 yields binary garbage, which the
  // model then "evaluated" and failed on every criterion. Extract the real text.
  if (name.endsWith('.docx') || contentType.includes('officedocument.wordprocessingml')) {
    try {
      const { value } = await mammoth.extractRawText({ buffer });
      const text = (value || '').trim();
      return text.length > 50 ? text : null;
    } catch (e) {
      console.error(`   ⚠️  docx parse failed for ${attachment.filename}: ${e.message}`);
      return null;
    }
  }

  const plain = buffer.toString('utf-8').replace(/[^\x20-\x7E\n]/g, ' ');
  // Binary formats (.doc, .pages) degrade into mostly-whitespace noise. Reject
  // rather than scoring a candidate on junk.
  const alpha = (plain.match(/[A-Za-z]/g) || []).length;
  if (alpha < plain.length * 0.35 || alpha < 200) {
    console.error(`   ⚠️  unreadable resume format (${attachment.filename}) — skipping`);
    return null;
  }
  return plain;
}

// ─── Claude scoring ───────────────────────────────────────────────────────────

const OPENROUTER_MODEL = process.env.OPENROUTER_MODEL || 'anthropic/claude-sonnet-5';
// Reasoning-capable models spend part of this budget on thinking before emitting
// content, so 800 truncated the JSON mid-string and silently dropped candidates.
const MAX_TOKENS = Number(process.env.OPENROUTER_MAX_TOKENS || 8000);
// Guard against pathological resumes (one parsed to 300k tokens and hard-failed).
const MAX_RESUME_CHARS = 120000;

async function scoreCandidate(resumeText, candidateName, jobConfig, attempt = 1) {
  const apiKey = process.env.OPENROUTER_API_KEY || process.env.ANTHROPIC_API_KEY;
  try {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        model: OPENROUTER_MODEL,
        max_tokens: MAX_TOKENS,
        messages: [
          {
            role: 'system',
            content: 'You are a recruiting assistant. Evaluate candidates accurately. For criteria with specific numeric thresholds (years of experience, headcount, percentages, quotas), be strict — only mark met if the resume clearly satisfies the number. For subjective or qualitative criteria, be reasonable — mark met if the resume reasonably suggests the candidate satisfies the intent. When a criterion says "note whether", treat it as met if the candidate has the attribute mentioned. Respond only with valid JSON.',
          },
          {
            role: 'user',
            content: `Evaluate this resume for a ${jobConfig.jobName} role.\n\n${jobConfig.criteriaPrompt}\n\nRespond ONLY with valid JSON:\n${jobConfig.jsonShape}\n\nResume for ${candidateName}:\n---\n${String(resumeText).slice(0, MAX_RESUME_CHARS)}`,
          },
        ],
      }),
    });
    if (!res.ok) {
      const errBody = await res.text();
      const e = new Error(`OpenRouter ${res.status}: ${errBody.slice(0, 200)}`);
      e.status = res.status;
      throw e;
    }
    const completion = await res.json();
    const choice = completion.choices?.[0];
    const finish = choice?.finish_reason;
    let text = (choice?.message?.content || '').trim();
    if (text.startsWith('```')) {
      text = text.replace(/^```(?:json)?\n?/, '').replace(/\n?```$/, '');
    }
    if (finish === 'length') {
      const e = new Error(`response truncated at max_tokens=${MAX_TOKENS} (finish_reason=length)`);
      e.truncated = true;
      throw e;
    }
    if (!text) throw new Error(`empty content (finish_reason=${finish})`);
    try {
      return JSON.parse(text);
    } catch (parseErr) {
      const e = new Error(`bad JSON (finish_reason=${finish}): ${parseErr.message} | raw: ${text.slice(0, 300)}`);
      e.badJson = true;
      throw e;
    }
  } catch (err) {
    const status = err.status || err.statusCode;
    const retryable = status === 429 || status === 500 || status === 502 || status === 503 || status === 529
      || err.truncated || err.badJson;
    if (attempt < 3 && retryable) {
      await new Promise(r => setTimeout(r, 5000 * attempt));
      return scoreCandidate(resumeText, candidateName, jobConfig, attempt + 1);
    }
    console.error(`❌ Scoring FAILED for ${candidateName} after ${attempt} attempt(s): ${err.message}`);
    return null;
  }
}

// ─── Slack ────────────────────────────────────────────────────────────────────

function formatSlackMessage(candidateName, candidateId, jobConfig, location) {
  const locationLine = location ? `\n${location}` : '';
  return `<https://app8.greenhouse.io/people/${candidateId}|${candidateName}>\n${jobConfig.jobName}${locationLine}`;
}

async function postToSlack(message) {
  const res = await fetch(process.env.SLACK_WEBHOOK_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ text: message }),
  });
  if (!res.ok) throw new Error(`Slack post failed: ${res.status}`);
}

// ─── Core job processor ───────────────────────────────────────────────────────

async function processJob(roleKey, timeWindowHoursOverride, { silent = false } = {}) {
  const jobConfig = JOB_CONFIGS[roleKey];
  if (!jobConfig) throw new Error(`Unknown role: ${roleKey}`);

  const timeWindowHours = timeWindowHoursOverride || CRITERIA_FILE.config?.timeWindowHours || 48;
  const since = new Date(Date.now() - timeWindowHours * 60 * 60 * 1000).toISOString();
  const results = {
    checked: 0, fetched: 0, skipped_wrong_stage: 0, skipped_work_auth: 0,
    skipped_no_resume: 0, skipped_score_error: 0, skipped_prior_rejection: 0,
    notified: 0, candidates: [],
  };

  const allApplications = [];
  for (const jobId of jobConfig.jobIds) {
    const apps = await fetchGreenhouseAll(
      `/applications?job_id=${jobId}&status=active&created_after=${since}&per_page=100`
    );
    allApplications.push(...apps);
  }
  const applications = allApplications;

  if (applications.length === 0) {
    if (!silent) await postToSlack(`No candidates found — ${jobConfig.jobName}`);
    return { message: 'No new applications in the last 48 hours.', ...results };
  }

  results.fetched = applications.length;

  const eligible = applications.filter(app => {
    if (!app.current_stage?.name?.toLowerCase().includes('application review')) {
      results.skipped_wrong_stage++;
      return false;
    }
    const { authorized } = isWorkAuthorized(app);
    if (!authorized) { results.skipped_work_auth++; return false; }
    return true;
  });

  const passing = [];

  for (const app of eligible) {
    const candidate = await fetchGreenhouse(`/candidates/${app.candidate_id}`);
    const candidateName = `${candidate.first_name} ${candidate.last_name}`;

    // Checked against the ATS before spending a scoring call on them.
    if (jobConfig.enforcePriorRejection) {
      const rejections = await getPriorRejections(candidate, app.id);
      if (rejections && rejections.length > 0) {
        results.skipped_prior_rejection++;
        console.log(`   ⛔ ${candidateName} — previously rejected ${rejections.length}x in Greenhouse, skipping`);
        continue;
      }
    }

    const resumeText = await parseResume(candidate);
    if (!resumeText) { results.skipped_no_resume++; continue; }

    let score;
    if (jobConfig.requiredCount === 0) {
      score = { passes: true };
    } else {
      await new Promise(r => setTimeout(r, 4000));
      score = await scoreCandidate(resumeText, candidateName, jobConfig);
      if (!score) { results.skipped_score_error++; continue; }
    }

    results.checked++;
    results.candidates.push({ name: candidateName, passes: score.passes });
    console.log(`   📋 ${candidateName} score:`, JSON.stringify(score, null, 2));

    if (score.passes) {
      const location = score.location || candidate.addresses?.[0]?.value || null;
      passing.push({ name: candidateName, id: candidate.id, location });
      if (passing.length >= 5) break;
    }
  }

  if (!silent) {
    if (passing.length > 0) {
      const lines = passing.map(c => `• <https://app8.greenhouse.io/people/${c.id}|${c.name}>${c.location ? ' — ' + c.location : ''}`);
      const message = `*${jobConfig.jobName} — ${passing.length} passed, ${results.checked} screened*\n${lines.join('\n')}`;
      await postToSlack(message);
      results.notified = passing.length;
    } else {
      await postToSlack(`No candidates found — ${jobConfig.jobName}`);
    }
  }

  results.jobName = jobConfig.jobName;
  results.passing = passing;
  return results;
}

module.exports = { processJob };
